...
function chkDupId(uid) {	// ajax를 이용한 아이디 중복체크를 위한 함수
		...
		$.ajax({
			...
			success : function(chkRs) {		// 데이터를 보내 실행한 결과를 chkRs로 받아옴
				if (chkRs == 0) {			// 사용할 수 있는 아이디 이면
					...
					$("#idChk").val("n");	// 아이디 중복체크를 성공한 상태로 변경
				}				
			}
		});
	} else {	// 사용자가 입력한 아이디가 4자 미만이면
		$("#idMsg").text("아이디는 4~20자 이내로 입력하세요.");	// 아이디 중복 체크를 하지 않거나 실패한 상태로 변경
		$("#idChk").val("n");
	}
}
</script>
...
<!-- 	
 ...
 -->
 	<input type="text" name="mi_id" id="mi_id" mexlength="20" onkeyup="chkDupId(this.value);"/>
