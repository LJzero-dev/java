...
String args = pageInfo.getArgs(), schargs = pageInfo.getSchargs(), schtype = pageInfo.getSchtype(), keyword = pageInfo.getKeyword();
%>
<style>
#list tr { height:30px; }
...
<tr align="center" onmouseover="this.bgColor='#efefef';" onmouseout="this.bgColor='';" >
	<td><%=num %></td><td align="left"><%=title %></td><td><%=fl.getFl_writer() %></td><td><%=fl.getFl_date() %></td><td><%=fl.getFl_read() %></td>
</tr>
<%	num--;	
	}
} else {	// 게시글 목록이 없으면
	...
	<tr>
		...
		<%
			if (rcnt > 0) {	// 게시글이 있으면 - 페이징 영역을 보여줌
				...
				for (int i = 1, j = spage; i <=bsize && j <= pcnt; i++, j++) {	// i : 블록에서 보여줄 페이지의 개수만큼 루프를 돌릴 조건으로 사용되는 변수
					if (cpage == j) {											// j : 실제 출력할 페이지 번호로 전체 페이지 개수(마지막 페이지 번호)를 넘지 않게 사용해야 함
