...
<tr align="center">
<td width="15%">
	<a href="productView?piid=<%=oc.getPi_id() %>"><img src="/mvcSite/product/pdt_img/<%=oc.getPi_img1() %>" width="100" height="95"></a>
</td>
...
<%
String maname = "", marname = "", maphone = "", mazip = "", maaddr1 = "", maaddr2 = "";	// 처음 페이지 로딩 시 보여줄 기본주소의 값들을 저장할 변수
for (MemberAddr ma : addrList) {
	if(ma.getMa_basic().equals("y")) {	// 기본주소이면
		maname = ma.getMa_name();	marname = ma.getMa_rname();	maphone = ma.getMa_phone();	mazip = ma.getMa_zip();	maaddr1 = ma.getMa_addr1();	maaddr2 = ma.getMa_addr2();
	}
	String val = "", txt = "";
...
<th width=15%>주소명</th>
<td width=35%>
	<input type="text" name="ma_name" value="<%=maname %>" readonly="readonly" style="border:none;" onfocus="this.blur();" />
</td>
...
%>
010 - <input type="text" name="p2" value="<%=arr[1] %>" size="4" maxlength="4" /> - <input type="text" name="p3" value="<%=arr[2] %>" size="4" maxlength="4" />
</td>
<th>우편번호</th>
<td>
...
<p style="width:800px; text-align:center"">
	<input type="radio" name="oi_payment" value="a" id="payA" checked="checked" /><label for="payA">카드 결제</label>&nbsp;&nbsp;&nbsp;&nbsp;
	<input type="radio" name="oi_payment" value="b" id="payB" /><label for="payB">휴대폰 결제</label>&nbsp;&nbsp;&nbsp;&nbsp;
	<input type="radio" name="oi_payment" value="c" id="payC" /><label for="payC">계좌 이체</label>
</p>
<hr />
<p style="width:800px; text-align:center">
