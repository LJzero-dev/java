...
public class ProductListCtrl extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String where = "", schargs = "";	// 검색조건 작업 : 대분류, 소분류, 가격대, 상품명, 브랜드
		...
		if (ob == null || ob.equals("")) ob = "a"; 	// ob가 없거나 빈문자일 경우
		...
		if (v == null || v.equals(""))	v = "l";	//	목록형(l)과 갤러리(g)형 모두 페이지 크기는 12로 동일함
		...
		ArrayList<ProductInfo> productList = productProcSvc.getProductList(cpage, psize, where, orderBy);	// 검색된 상품들 중 현재 페이지에서 보여줄 상품 목록을 받아옴
		
		ArrayList<ProductCtgrSmall> smallList = new ArrayList<ProductCtgrSmall>();
		if (pcb != null && !pcb.equals("")) 	// 검색조건 중 대분류가 있으면
			smallList = productProcSvc.getCtgrSmallList(pcb);	// 검색조건의 대분류에 속하는 소분류 목록을 받아옴
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("/product/product_list.jsp");	// 글 등록 전용 폼이므로 따로 Service나 Dao 클래스 없이 바로 View에 해당하는 jsp 파일로 이동
