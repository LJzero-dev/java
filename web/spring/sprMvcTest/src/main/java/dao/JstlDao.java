...
public class JstlDao {
	...
	public List<MemberInfo> getMemberList(PageInfo pi) {
		String where = "";
		if (pi.getSchtype() != null && !pi.getSchtype().equals("") && pi.getKeyword() != null && !pi.getKeyword().equals("")) {
			where += " where mi_" + pi.getSchtype() + " like '%" + pi.getKeyword() + "%'";
		}
		
		List<MemberInfo> memberList = jdbc.query(
				"select * from t_member_info " + where, (ResultSet rs, int rowNum) -> {
