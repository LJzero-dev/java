...
public class MainForImport {
	...
	private static void processNewCommand(String[] arr) {
		...
		MemberRegisterSvc regSvc = ctx.getBean("memberRegSvc", MemberRegisterSvc.class);	// 앞이 메소드 이름
	...
	private static void processChangeCommand(String[] arr) {
		...
		try {
		ChangePasswordSvc pwdSvc = ctx.getBean("changePwdSvc", ChangePasswordSvc.class);	//	스프링 컨테이너로부터 이름이 'changePwdSvc'인 빈 객체를 주입받음
