...
int sd = ci.getSchDay(), eDay = ci.getSchLast();
%>
...
<th width="15%">일시</th>
<td width="*">
	<select name="y" id="y" onchange="resetday(this.value, 
		this.form.m.value, this.form.d);">
<% for (int i = 2000 ; i <= ci.getCurYear() + 10 ; i++) { %>
		<option <% if(i == sy) { %>selected="selected"<% } %>><%=i %></option>
<% } %>
	</select>년
	<select name="m" id="m" onchange="resetday(this.form.y.value, 
		this.value, this.form.d);">
<%
for (int i = 1 ; i <= 12 ; i++) {
